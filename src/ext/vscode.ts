...
export class VSCode {
  ...
  public getUserInputWithValidation(): Q.Promise<J.Model.Input> {
    ...
    try {
      ...
      input.items = this.getInputItems(null);
      ...
      input.onDidChangeValue((val) => {
        ...
        if (val.length == 0) {
          ...
          this.ctrl.parser.parseInput(val).then((parsed: J.Model.Input) => {
            ...
            input.items = this.getInputItems(parsed);
          });
        }
      }, disposables);
      ...
      input.onDidAccept((val) => {
        ...
        if (J.Util.isNotNullOrUndefined(selected.parsedInput)) {
          ...
          J.Util.isNotNullOrUndefined(selected.pickItem) &&
          selected.pickItem == JournalPageType.ENTRY
        ) {
          ...
          J.Util.isNotNullOrUndefined(selected.pickItem) &&
          selected.pickItem == JournalPageType.NOTE
        ) {
          ...
          J.Util.isNotNullOrUndefined(selected.pickItem) &&
          selected.pickItem == JournalPageType.ATTACHEMENT
        ) {
  ...
  private getInputItems(input: J.Model.Input | null): DecoratedQuickPickItem[] {
    const scope = this.ctrl.getScope(input);
    const scopeSuffix = scope ? ` [${scope}]` : "";

    const result: DecoratedQuickPickItem[] = [];
    if (input) {
      let item: DecoratedQuickPickItem = {
        label: input.rawValue,
        path: "",
        alwaysShow: true,
        parsedInput: input,
        replace: true,
        description: input.generateDescription(this.ctrl.config),
        detail: input.generateDetail(this.ctrl.config),
      };
      result.push(item);
    }

    let today: DecoratedQuickPickItem = {
      label: this.ctrl.config.getInputLabelTranslation(1) + scopeSuffix,
      description: this.ctrl.config.getInputDetailsTranslation(1),
      pickItem: JournalPageType.ENTRY,
      parsedInput: new J.Model.Input(0, scope),
      alwaysShow: true,
      path: "",
    };
    let tomorrow: DecoratedQuickPickItem = {
      label: this.ctrl.config.getInputLabelTranslation(2) + scopeSuffix,
      description: this.ctrl.config.getInputDetailsTranslation(2),
      pickItem: JournalPageType.ENTRY,
      parsedInput: new J.Model.Input(1, scope),
      alwaysShow: true,
      path: "",
    };
    let pickEntry: DecoratedQuickPickItem = {
      label: this.ctrl.config.getInputLabelTranslation(3) + scopeSuffix,
      description: this.ctrl.config.getInputDetailsTranslation(3),
      pickItem: JournalPageType.ENTRY,
      parsedInput: new J.Model.Input(2, scope),
      alwaysShow: true,
      path: "",
    };
    let pickNote: DecoratedQuickPickItem = {
      label: this.ctrl.config.getInputLabelTranslation(4) + scopeSuffix,
      description: this.ctrl.config.getInputDetailsTranslation(4),
      pickItem: JournalPageType.NOTE,
      parsedInput: new J.Model.Input(3, scope),
      alwaysShow: true,
      path: "",
    };
    
    result.push(...[today, tomorrow, pickEntry, pickNote]);
  ...
  public addItem(
    fe: FileEntry,
    input: vscode.QuickPick<DecoratedQuickPickItem>,
    ...
    input.items = input.items
      .concat(item)
      .sort((a, b) => b.fileEntry!.created_at - a.fileEntry!.created_at);
    /*
  ...
  public pickItem(type: JournalPageType) {
    ...
    try {
      ...
      this.ctrl.reader.getPreviouslyAccessedFiles(
        this.ctrl.config.getInputTimeThreshold(),
        this.addItem,
        input,
        type,
        baseDirectories
      ...
      this.ctrl.reader
        .getPreviouslyAccessedFilesSync(
          this.ctrl.config.getInputTimeThreshold(),
          baseDirectories
        )
      ...
      if (type == JournalPageType.NOTE) {
        input.onDidChangeValue((val) => {
          if (val.length == 0) {
            ...
            this.ctrl.parser
              .resolveNotePathForInput(inputText, scope)
              .then((path) => {
                inputText.path = path;

                console.log(
                  "Tags in input string: " +
                    inputText.tags +
                    " and scope " +
                    inputText.scope
                );
                ...
                if (inputText.scope == SCOPE_DEFAULT) {
                  ...
                  description =
                    'Create new note in scope "' + inputText.scope + '"';
                }
                ...
                if (
                  ...
                  input.items[0].replace &&
                  input.items[0].replace === true
                ) {
          ...
          textDocument.save().then(
            (isSaved) => {
              if (isSaved === true) {
                ...
                reject(
                  "Failed to save file with path: " + textDocument.fileName
                );
              }
            },
            (rejected) => {
  ...
  public showDocument(
    textDocument: vscode.TextDocument
  ): Q.Promise<vscode.TextEditor> {
    this.ctrl.logger.trace(
      "Entering showDocument() in ext/vscode.ts for document: ",
