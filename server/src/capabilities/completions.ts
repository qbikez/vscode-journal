...
export class JournalCompletions {
    ...
    private listen() {
        ...
        this.connection.onCompletion((_pos: vscode.TextDocumentPositionParams): vscode.CompletionItem[] => {
            // The pass parameter contains the position of the text document in 
            // which code complete got requested. For the example we ignore this
            // info and always provide the same completion items.

            // get text of line
            let doc: vscode.TextDocument = this.documents.get(_pos.textDocument.uri);

            // linenumber


            let lines: string[] = doc.getText().split(/\r?\n/g);
            let currentLine: string = lines[_pos.position.line];
            ...
            let replaceRange: vscode.Range = vscode.Range.create(vscode.Position.create(_pos.position.line, 3), vscode.Position.create(_pos.position.line, 4));
            ...
            } else this.connection.console.log("No match");
        ...
        // the completion list.
        this.connection.onCompletionResolve((item: vscode.CompletionItem): vscode.CompletionItem => {
            if (item.data === 1) {
